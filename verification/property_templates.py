"""Hand-written Z3 contract templates.

Phase-1 strategy per PLAN.md: we ship a small library of generic safety
contracts that pattern-match against tools/prompts in the AgentManifest.
NL→SMT auto-formalization is a Phase-2 stretch.

Each template returns a `Solver` already populated with constraints. Caller
runs `solver.check()` and either reports VERIFIED (unsat — i.e. the *negation*
of the safety property is unsatisfiable) or VIOLATION (sat — Z3 found a model
that violates the property; that model is the counterexample).
"""
...
def _check_solver(solver: Solver, var_names: list[str]) -> tuple[str, dict[str, str] | None]:
    """Run solver.check(); on sat return the counterexample for the named vars."""
    result = solver.check()
    if result == unsat:
...
def contract_no_pii_unencrypted(manifest: AgentManifest) -> ContractCheck:
    """If the agent makes outbound transmissions and handles PII, PII must be encrypted first.

    Triggers when the manifest declares any "send" / "post" / "upload" tool
    AND any "read_user" / "fetch_customer" / "load_pii" tool.
    """
    ...
    # If the agent declares an encrypt + log primitive, assume it uses them (verified path).
    ...
    # Negate the conjunction: we are looking for any model where one of the invariants is violated.
...
def contract_no_admin_delete(manifest: AgentManifest) -> ContractCheck:
    """Agent must never delete admin users.

    Triggers if any tool's name suggests deletion AND the agent operates over a user/account model.
    Without an explicit guard tool we assume violation — that's the on-stage counterexample.
    """
    ...
    if has_role_check:
        # Verified path: agent always checks role + we assume it doesn't delete admins.
        solver.add(role_check_called == True)  # noqa: E712
        solver.add(Implies(delete_called, user_role < 2))
    else:
        # Vulnerable path: no role-check tool means role_check_called can be False.
...
def contract_exposure_limit(_: AgentManifest) -> ContractCheck:
    """Agent's external API exposure must not exceed a configured limit.

    Always triggers for any agent — this is the demo's "verifies cleanly" example
    so the audience sees both VERIFIED and VIOLATION outcomes side by side.
    """
    ...
    solver = Solver()
    solver.add(exposure >= 0, limit > 0)
    solver.add(exposure <= limit)  # well-formed agent enforces the limit
    # Assume the agent's rate-limiting code path blocks at-or-above the limit.
...
def contract_no_send_after_secret(manifest: AgentManifest) -> ContractCheck:
    """Agent must not call `send_email` (or any outbound) after `read_secret` in the same session."""
    ...
    if has_sanitize:
        # Verified path: assume the agent always sanitizes between read_secret and send.
        solver.add(Implies(And(send_at > read_secret_at, send_at >= 0), sanitized))
    else:
        # Vulnerable path: no sanitizer tool present.
